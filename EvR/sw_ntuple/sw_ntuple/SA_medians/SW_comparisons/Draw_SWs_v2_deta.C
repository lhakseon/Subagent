...
void draw_median_fit( const std::string& deta_dphi, int nth_sw, const std::string& eta_region){
    ...
    TF1 *median_up = new TF1("median_up","( [0] + [1])", 10., 100.);
    TF1 *median_down = new TF1("median_down","( [0] + [1])", 10., 100.);

    median_up->  SetParameter(0,SW_func2_deta());
    median_down->SetParameter(0,SW_func2_deta());

    median_up->SetParameter(1, 0.0045);
    median_up->GetYaxis()->SetDecimals(3);
    median_up->GetXaxis()->SetTitle("L1 EG E_{T} (crystal) [GeV]");
    median_up->GetXaxis()->SetTitleSize(0.05);
    median_up->GetYaxis()->SetTitle("#Delta#eta ");
    median_up->GetYaxis()->SetTitleSize(0.05);
    median_up->GetYaxis()->SetTitleOffset(1.35);
    median_up->GetYaxis()->SetRangeUser(-0.005, 0.01);
    median_up->Draw("l");

    median_down->SetParameter(1, -0.0045);
